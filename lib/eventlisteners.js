    ...
    if (functions.isMasterProcess()) {
      client.on('message', msg => {
        ...
        if (authorId != botDiscordId) {
          ...
          // C
          else if (_.isEqual(textSplit[0], "getlink")) {
            if (member.roles.has(chiefRoleId) || member.roles.has(superBARoleId) || member.roles.has(hellBARoleId)) {
              msg.delete(500)
                .then(msg => console.log(`Deleted !resend from ${msg.author.username}`))
                .catch(function(err) {
              ...
              client.channels.get(msg.channel.id).send(`Please join our discord: https://discord.gg/hBqcPfU When you enter type ${raidPass} in #general-public chat to be able to join the raid voice chat. Room is called wow-raid`);
            } else {
              client.channels.get(msg.channel.id).send("Insufficient permissions!");
            }
          }
      ...
      client.on('guildMemberAdd', member => {
        client.channels.get(publicChannelId).send("Hi!. If you are here to join the raid. Please type the join key given in Instance chat in #public-general. Then join the voice channel: wow-raid");
      })
    }
  }
}
